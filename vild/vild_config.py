...
class AudioViLDConfig:
    def __init__(self, mark_version="mark4.1"):  # change needed
        ...
        # [갱신 2026-08-09 (2) / mark4.5] 0.31 -> 0.47. **train 세션 편중을 없애고 재수집한
        # 데이터로 다시 학습한 모델** 기준이라 아래 (1) 의 0.31 은 더는 유효하지 않다.
        # val(430개) 스윕 결과 accuracy 최고 0.9907 플래토가 (0.4445, 0.4944] 하나뿐이고
        # 폭 0.0499, 중앙 0.4695 라 가장 가까운 격자값 0.47 을 채택. 0.31 로 두면 acc 0.9791 로
        # 오탐이 1건에서 6건으로 늘어난다. others 최고 확률 0.6139(CHiME 아동 말소리) 하나는
        # 어떤 값으로도 못 거른다.
        # val 성능: accuracy 0.9907, macro F1 0.9907, media_talking R 0.986, others FPR 0.005,
        # ROC AUC 0.9995. 재현 검증은 thr 0.31 로 430/430 일치.
        # 1차 대비 개선: 「내용이 쓰는 비율」 9.6% -> 16.3%, student best epoch 14 -> 29,
        # student val_best loss 0.2247 -> 0.2164. 데이터가 어려워졌는데 최종 손실은 좋아졌다.
        #
        # [갱신 2026-08-09 (1) / mark4.5] 0.45 -> 0.31. ⚠아래는 세션 편중이 있던 1차 데이터
        # 기준이라 기록으로만 남긴다. 그 구성은 val 0.9930 이었으나 test 0.8953 으로 9.8%p 격차가
        # 났고, 원인이 CHiME train 타겟의 88.8% 가 한 세션에서 온 것이었다(SINS 도 3세션 편중).
        # threshold 손실은 0.0%p 였고 격차 전량이 데이터 난이도였다.
