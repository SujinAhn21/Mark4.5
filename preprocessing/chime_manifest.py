...
# 세션 → split. 2026-08-09 확정, 같은 날 1차 학습 결과를 보고 개정.
SESSION_SPLIT = {
    "CR_lounge_270110_1632": "train",
    "CR_lounge_220110_0731": "train",
    "CR_lounge_200110_1601": "train",   # [추가 2026-08-09] 타겟 편중 세션(97.7%)이지만 train 에 넣는다
    "CR_lounge_230110_1501": "train",   # [추가 2026-08-09] others 편중 세션(0.3%), 위와 짝을 이룬다
    "CR_lounge_210110_0739": "val",
    "CR_lounge_200110_1711": "test",
    "CR_lounge_230110_1036": "test",
}
# [개정 근거 2026-08-09] 처음에는 편중 세션 둘을 아예 뺐다. 그랬더니 train 타겟 250개 중
# 222개(89%)가 270110_1632 한 세션에서 나왔고, 학습이 사실상 그 세션의 TV 소리만 보게 됐다.
# 그 결과 test 에서 CHiME 이 val 0.9720 -> test 0.7963 으로 17.6%p 떨어졌다(오분류 45건 중
# 43건이 CHiME, 그중 28건이 200110_1711 의 TV 를 놓친 것). SINS 쪽은 val 1.0000 -> test 0.9953
# 으로 멀쩡했으므로 격차 전량이 CHiME 에서 나왔다.
#
# 편중 세션을 train 에 넣으면 train 안에서 세션이 클래스와 부분적으로 붙지만, 두 세션이 서로
# 반대 방향(타겟 편중 / others 편중)이라 상쇄되고, 무엇보다 **검출은 val·test 에서 한다**.
# val(210110_0739)과 test(200110_1711·230110_1036)는 여전히 두 클래스를 넉넉히 가진 균형
# 세션이므로, train 에서 세션 단서를 배웠다면 거기서 드러난다.
#
# ⚠ test 세션은 바꾸지 않는다. 어려운 세션(200110_1711)을 train 으로 옮기면 숫자는 오르지만
#   그것은 평가를 쉽게 만드는 것이다. train 다양성만 늘리고 평가 조건은 그대로 둔다.
#
# ⚠ 세션 음향 거리(40밴드 로그멜 통계)로 난이도를 예측해 배정을 최적화하려 했으나 실패했다.
#   200110_1711 은 train 과 **가장 가깝고**(중심거리 5.7) v/others 분리도도 중간(1.12)인데
#   실측 정확도가 0.6778 로 최악이었고, 230110_1036 은 train 에서 가장 멀고(9.2) 분리도도
#   최악(0.51)인데 0.8810 이 나왔다. 순서가 뒤집혀 있다. 거친 스펙트럼 통계는 모델이 보는
#   것(학습된 임베딩과 텍스트 프롬프트의 코사인 유사도)과 다르다. 그래서 배정은 거리가 아니라
#   **원칙**으로 정한다 — train 은 세션이 고르게 많을수록 좋고, test 는 건드리지 않는다.
